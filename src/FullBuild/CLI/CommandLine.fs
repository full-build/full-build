...
let UsageContent() =
    let content = [
        ...
        [MainCommand.Workspace; MainCommand.Setup], "setup <git|gerrit|svn> <master-repository> <master-artifacts> <local-path> : setup a new environment in given path"
        ...
        [MainCommand.View; MainCommand.BuildView], "build [--mt] [--platform <platform] [--config <config>] [--version <version>] [<viewId>] : build view"
        [MainCommand.View; MainCommand.RebuildView], "rebuild [--mt] [--debug] [--version <version>] [<viewId>] : rebuild view (clean & build)"
        [MainCommand.View; MainCommand.Test], "test <viewId> : test view"
